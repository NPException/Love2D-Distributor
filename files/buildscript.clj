...
(def game-name     "My Game")
(def osx-bundle-id "com.MyCompany.MyGame")

(def src-folder    (io/file "./src"))
(def release-folder (io/file "./dist"))

(def love-win64 (io/file "./lovefiles/win64"))
(def love-osx   (io/file "./lovefiles/love.app"))
...
;; TODO: create OSX release

(if-not (.exists love-osx)
  (println "No OSX Love2D binaries found.")
  (let [target-folder (io/file release-folder "osx")
        app-folder (io/file target-folder (str game-name ".app"))]
    (when (.exists target-folder)
      (delete! target-folder))
    ;; copy .app folder
    (copy! love-osx app-folder)
    (copy! love-file (io/file app-folder (str "Contents/Resources/" (fname love-file))))
    ;; read and modify content of plist file
    (->> (io/file app-folder "Contents/Info.plist")
         slurp
         str/split-lines
         (reduce
           (fn [[state mod-lines] line]
             ; TODO
             )
           [:plist/searching []]))
    ))
